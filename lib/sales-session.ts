...
type SessionData={userId:string;salesPersonId:string;name:string;permissions:SalesPermission[];expiresAt:number};
...
export async function createSalesSession(user:{id:string;salesPersonId:string;name:string;permissions?:SalesPermission[]}){const payload=encode(JSON.stringify({userId:user.id,salesPersonId:user.salesPersonId,name:user.name,permissions:normalizeSalesPermissions(user.permissions),expiresAt:Date.now()+salesSessionMaxAge*1000}));return `${payload}.${await sign(`sales-session:${payload}`)}`}
export async function readSalesSession(value?:string):Promise<SessionData|null>{if(!value)return null;const[payload,sig]=value.split(".");if(!payload||!sig||!equal(await sign(`sales-session:${payload}`),sig))return null;try{const data=JSON.parse(decode(payload)) as SessionData;return data.expiresAt>Date.now()?{...data,permissions:normalizeSalesPermissions(data.permissions)}:null}catch{return null}}
