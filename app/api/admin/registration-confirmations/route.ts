...
import type { RegistrationConfirmationActivity, RegistrationEntry } from "@/lib/types";

export const runtime = "nodejs";

export async function PATCH(request: Request) {
  const database = getDbPool();
  if (!database) return NextResponse.json({ error: "Database is required to confirm registrations." }, { status: 503 });
  ...
  try {
    ...
    if (!workshopId || !ids.length) {
      return NextResponse.json({ error: "Workshop and registrations are required." }, { status: 400 });
    }
    ...
    try {
      ...
      for (const { payload: entry } of selected.rows) {
        if (entry.confirmationStatus === "confirmed" && entry.registrationStatus !== "waiting") continue;
        ...
        const activity: RegistrationConfirmationActivity = {
          ...
          status: "confirmed",
          actorGrantId: "admin",
          actorName: "Workshop Master Admin",
          createdAt: now
        };
        const next: RegistrationEntry = {
          ...entry,
          confirmationStatus: "confirmed",
          confirmationSource: entry.confirmationSource ?? "manual",
          confirmationUpdatedAt: now,
          confirmationUpdatedBy: "Workshop Master Admin",
          confirmationHistory: [activity, ...(entry.confirmationHistory ?? [])].slice(0, 200),
      ...
      if (!confirmed) {
        ...
        return NextResponse.json({ error: "The selected registrations are already confirmed." }, { status: 409 });
      }
    ...
    return NextResponse.json({ confirmed, promoted, registrations: result.rows.map((row) => row.payload), scope: "workshop", workshopId });
  } catch (error) {
    console.error("Bulk registration confirmation failed", error instanceof Error ? error.message : "unknown");
    return NextResponse.json({ error: "Could not confirm the selected registrations." }, { status: 500 });
  }
}
