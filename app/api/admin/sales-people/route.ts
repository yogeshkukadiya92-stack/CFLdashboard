...
export async function POST(request: Request) {
  try {
    ...
    const roles=existing?normalizeCrmTeamRoles(existing.roles):["sales"] as const;const user:SalesTeamUser={id:existing?.id||`sales-user-${crypto.randomUUID()}`,salesPersonId:String(salesPerson.id),name:String(salesPerson.name),email:String(salesPerson.email||"").toLowerCase(),mobile,passwordHash:password?hashSalesPassword(password,mobile):existing!.passwordHash,active:salesPerson.isActive!==false,canViewOther:Boolean(salesPerson.canViewOther),roles:[...roles],permissions:permissionsAllowedForRoles(existing?.permissions,roles),createdAt:existing?.createdAt||now,updatedAt:now,lastLoginAt:existing?.lastLoginAt,loginCount:existing?.loginCount||0};
