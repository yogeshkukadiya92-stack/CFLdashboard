...
import { permissionsAllowedForRoles } from "@/lib/sales-permissions";
...
export async function POST(request:NextRequest){
  ...
  if(!user||!user.active||!verifySalesPassword(String(body.password||""),user))return NextResponse.json({error:"Invalid email/mobile number or password."},{status:401});const updated={...user,lastLoginAt:new Date().toISOString(),loginCount:user.loginCount+1};const permissions=permissionsAllowedForRoles(updated.permissions,updated.roles);const home=permissions.includes("crm_today")?"/crm/today":permissions.includes("sales_sessions")?"/crm/sessions":permissions.includes("follow_ups")?"/crm/follow-ups":permissions.includes("crm_analytics")?"/crm/analytics":"/sales-login?access=none";await saveAppState({salesTeamUsers:[updated,...users.filter((u)=>u.id!==user.id)]});const response=NextResponse.json({ok:true,name:user.name,home});response.cookies.set({name:SALES_SESSION_COOKIE,value:await createSalesSession(updated),httpOnly:true,secure:process.env.NODE_ENV==="production",sameSite:"lax",path:"/",maxAge:salesSessionMaxAge});return response;}catch{return NextResponse.json({error:"Could not sign in."},{status:500})}
}
