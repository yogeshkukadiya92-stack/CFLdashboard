...
const stats = [
  { icon: CalendarDays, label: "Total Workshops", value: "0", tone: "bg-indigo-50 text-indigo-700" },
  { icon: TrendingUp, label: "Upcoming Workshops", value: "0", tone: "bg-cyan-50 text-cyan-700" },
  { icon: UsersRound, label: "Total Members", value: "0", tone: "bg-rose-50 text-rose-700" },
  { icon: IndianRupee, label: "Revenue Collected", value: "INR 0", tone: "bg-emerald-50 text-emerald-700" }
];
...
export default function DashboardPage() {
  return (
    <AdminPlatformShell
      ...
      description="Clean live workspace. Upload your workshop, client and payment data to start analytics."
      ...
      <div className="grid gap-4 xl:grid-cols-[1.2fr_0.8fr]">
        <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <div className="mt-3 flex flex-wrap items-end justify-between gap-4">
            <div>
              <p className="text-5xl font-black tracking-tight text-slate-950">0</p>
              <p className="mt-2 text-sm font-semibold text-slate-500">Upload data to start revenue analytics.</p>
            </div>
            <div
            ...
            >
              <div className="grid size-28 place-items-center rounded-full bg-white shadow-inner">
                <span className="text-lg font-black text-slate-950">INR 0</span>
              </div>
            </div>
          </div>
          <div className="mt-5 grid gap-2 sm:grid-cols-3">
            {["Category 1 0%", "Category 2 0%", "Category 3 0%"].map((item) => (
        ...
        <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <div className="mt-5 rounded-3xl border border-indigo-100 bg-indigo-50 p-5">
            <span className="rounded-xl bg-indigo-600 px-3 py-2 text-sm font-black text-white">No Date</span>
            <h3 className="mt-5 text-2xl font-black text-slate-950">No upcoming event</h3>
            <p className="mt-2 text-sm text-slate-600">Upload workshop data to show schedules.</p>
            <a className="mt-5 inline-flex rounded-xl bg-slate-950 px-4 py-3 text-sm font-bold text-white" href="/process/import-data-workshop-wise">
              Upload Data
            </a>
          </div>
        </section>
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <div className="mt-4 overflow-x-auto">
          <table className="min-w-[760px] w-full text-left text-sm">
            ...
            <tbody>
              <tr>
                <td className="px-4 py-8 text-center text-slate-500" colSpan={5}>No event data uploaded yet.</td>
              </tr>
            </tbody>
          </table>
        </div>
