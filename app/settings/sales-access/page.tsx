...
import { salesPermissionOptions } from "@/lib/sales-permissions";
import type { SalesPermission } from "@/lib/types";
import { Check, LockKeyhole, Save, ShieldCheck, UserRound } from "lucide-react";
import { useEffect, useState } from "react";

type AccessUser = { id: string; name: string; email: string; mobile: string; active: boolean; permissions: SalesPermission[] };
...
export default function SalesAccessPage() {
  ...
  useEffect(() => {
    fetch("/api/admin/sales-access", { cache: "no-store" }).then(async (response) => {
      ...
      setUsers(next);
      if (next[0]) { setSelectedId(next[0].id); setPermissions(next[0].permissions); }
    }).catch(() => setMessage("Could not load salesperson access." )).finally(() => setLoading(false));
  }, []);

  function selectUser(user: AccessUser) { setSelectedId(user.id); setPermissions(user.permissions); setMessage(""); }
  ...
  async function save() {
    ...
    try {
      const response = await fetch("/api/admin/sales-access", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ userId: selected.id, permissions }) });
      const data = await response.json();
      if (!response.ok) throw new Error(data.error);
      setUsers((current) => current.map((user) => user.id === selected.id ? { ...user, permissions } : user));
      setMessage("Access saved. Changes apply after the salesperson signs in again.");
    } catch (error) { setMessage(error instanceof Error ? error.message : "Could not save access."); }
    finally { setSaving(false); }
  }

  return <AdminPlatformShell activeLabel="Sales Access" title="Sales Access Management" description="Choose exactly which CRM screens each salesperson can open. Access is enforced by the server.">
    <div className="grid gap-4 xl:grid-cols-[340px_minmax(0,1fr)]">
      <section className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
        <div className="flex items-center gap-3 border-b border-slate-100 px-1 pb-4"><span className="grid size-10 place-items-center rounded-xl bg-slate-950 text-white"><UserRound className="size-5" /></span><div><h3 className="font-black text-slate-950">Sales team</h3><p className="text-xs font-semibold text-slate-500">{users.length} login accounts</p></div></div>
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm md:p-6">
        ...
        <div className="mt-6 grid gap-3 md:grid-cols-2">{salesPermissionOptions.map((option) => { const checked=permissions.includes(option.key); return <button aria-pressed={checked} className={`flex min-h-28 items-start gap-3 rounded-2xl border p-4 text-left transition ${checked ? "border-emerald-300 bg-emerald-50" : "border-slate-200 bg-white hover:bg-slate-50"}`} key={option.key} onClick={() => toggle(option.key)} type="button"><span className={`mt-0.5 grid size-7 shrink-0 place-items-center rounded-lg ${checked ? "bg-emerald-600 text-white" : "bg-slate-100 text-slate-400"}`}>{checked ? <Check className="size-4" /> : <LockKeyhole className="size-4" />}</span><span><strong className="block text-sm font-black text-slate-950">{option.label}</strong><span className="mt-1 block text-xs font-semibold leading-5 text-slate-500">{option.description}</span></span></button>})}</div></> : <div className="grid min-h-72 place-items-center text-center"><div><UserRound className="mx-auto size-10 text-slate-300" /><p className="mt-3 font-black text-slate-700">Select a salesperson</p></div></div>}
      </section>
    </div>
  </AdminPlatformShell>;
}
