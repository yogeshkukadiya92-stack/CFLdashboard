...
export default function AttendanceFormPage() {
  ...
  useEffect(() => {
    ...
    if (!session || mobile.length !== 10 || !/^[6-9]/.test(mobile)) return;

    const controller = new AbortController();
    const timer = window.setTimeout(async () => {
      try {
        const response = await fetch(`/api/public-profile-lookup?type=attendance&slug=${encodeURIComponent(slug)}&mobile=${mobile}`, {
          cache: "no-store",
  ...
  async function submitAttendance() {
    ...
    const entry: AttendanceEntry = {
      ...
      id: session.allowDuplicate ? `att-${session.id}-${mobile || "guest"}-${generateId()}` : `att-${session.id}-${mobile || generateId()}`,
        ...
        <div aria-modal="true" className="fixed inset-0 z-50 grid place-items-center bg-slate-950/60 p-4 backdrop-blur-sm" role="dialog">
          <section className="attendance-success-modal w-full max-w-md overflow-hidden border border-slate-200 bg-white shadow-2xl">
            ...
            <div className="p-6 text-center sm:p-8">
              ...
              <p className="mt-2 text-xs font-semibold text-slate-400">{session.allowDuplicate ? "Multiple responses are enabled for this form." : "A mobile number can be recorded once for this session."}</p>
            </div>
          </section>
        </div>
      ) : null}
...
function RenderField({
  ...
  } else if (isHeightField(field.type, field.label)) {
  ...
  if (isHeightField(field.type, field.label)) return <div className={`block ${wide ? "md:col-span-2" : ""}`}>{content}</div>;
