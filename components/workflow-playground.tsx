...
export function WorkflowPlayground({ initialWorkflowId = "workshop-registration-onboarding", fullScreen = false, onExit, onOpenWorkflow }: {
  ...
  const [attendanceSessions, setAttendanceSessions] = useState<WorkflowAttendanceSession[]>([]);
  const [workshops, setWorkshops] = useState<WorkflowWorkshop[]>([]);
  ...
  useEffect(() => {
    ...
    async function hydrateFromServer() {
      try {
        ...
        const payload = await response.json() as { workflow?: { id?: string; name?: string; status?: string; version?: number; nodes?: WorkflowNode[]; connections?: Connection[]; note?: string }; executions?: RunRow[]; salesPeople?: WorkflowSalesPerson[]; attendanceSessions?: WorkflowAttendanceSession[]; workshops?: WorkflowWorkshop[]; whatsapp?: WhatsAppAutomationOverview; attendance?: AttendanceAutomationOverview; payment?: PaymentAutomationOverview; crm?: CrmAutomationOverview; versions?: WorkflowVersionSummary[]; reliability?: WorkflowReliabilityOverview; schedules?: WorkflowScheduleOverview; governance?: WorkflowGovernanceOverview; incidents?: WorkflowIncidentOverview; enterprise?: WorkflowEnterpriseOverview };
        if (cancelled) return;
        ...
        setAttendanceSessions(Array.isArray(payload.attendanceSessions) ? payload.attendanceSessions : []);
    ...
    <div className={`relative flex min-w-0 ${focusMode ? "min-h-0 flex-1" : "min-h-[720px]"}`}>
      ...
      <main className="flex min-w-0 flex-1 flex-col bg-slate-100">
        ...
        <div className={`relative overflow-hidden ${focusMode ? "min-h-0 flex-1" : "h-[672px]"} ${spacePressed || panning ? "cursor-grabbing" : "cursor-grab"}`} onPointerDown={onCanvasPointerDown} onPointerMove={onCanvasPointerMove} onPointerUp={() => { setDragging(null); setPanning(null); dragHistoryCapturedRef.current = false; }} onWheel={onCanvasWheel} ref={canvasRef} style={{ touchAction: "none" }}>
          ...
          {connectingFrom ? <div className="absolute left-1/2 top-3 z-30 flex -translate-x-1/2 items-center gap-2 rounded-xl border border-indigo-200 bg-white px-3 py-2 text-[11px] font-black text-indigo-700 shadow-lg"><span className="size-2 animate-pulse rounded-full bg-indigo-500" />Connecting from {nodeMap.get(connectingFrom)?.title ?? "node"} · click the next node<button aria-label="Cancel connection" className="ml-1 rounded-md p-1 text-slate-400 hover:bg-slate-100" onClick={(event) => { event.stopPropagation(); setConnectingFrom(null); }} type="button"><X className="size-3.5" /></button></div> : null}
...
function CanvasNode({ connectingFrom, executionStatus, node, onConnect, onKeyboardSelect, onOpenPicker, onPointerDown, selected }: { connectingFrom: string | null; executionStatus: ExecutionNodeStatus; node: WorkflowNode; onConnect: () => void; onKeyboardSelect: () => void; onOpenPicker: () => void; onPointerDown: (event: ReactPointerEvent<HTMLDivElement>) => void; selected: boolean }) {
  const colors = colorByKind[node.kind];
  const isConnectionSource = connectingFrom === node.id;
  const isConnectionTarget = Boolean(connectingFrom) && !isConnectionSource;
  const statusClass = isConnectionSource ? "border-indigo-600 ring-4 ring-indigo-100" : isConnectionTarget ? "border-indigo-300 border-dashed ring-4 ring-indigo-50 hover:border-indigo-600 hover:ring-indigo-100" : executionStatus === "success" ? "border-emerald-500 ring-4 ring-emerald-100" : executionStatus === "running" ? "border-indigo-500 ring-4 ring-indigo-100" : executionStatus === "failed" ? "border-rose-500 ring-4 ring-rose-100" : selected ? "border-indigo-500 ring-4 ring-indigo-100" : colors.border;
  return <div aria-label={`${node.title} workflow node${isConnectionTarget ? " · click to connect" : ""}`} className={`group absolute h-[112px] w-[188px] select-none rounded-2xl border-2 bg-white shadow-[0_12px_30px_-18px_rgba(15,23,42,0.55)] transition-[box-shadow,border-color,transform] hover:-translate-y-0.5 hover:shadow-[0_18px_36px_-18px_rgba(15,23,42,0.5)] ${statusClass} ${connectingFrom ? "cursor-crosshair" : "cursor-grab active:cursor-grabbing"}`} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); onKeyboardSelect(); } }} onPointerDown={onPointerDown} role="button" style={{ left: node.x, top: node.y }} tabIndex={0}><div className={`flex h-full flex-col p-3 ${isConnectionTarget ? "opacity-45 transition-opacity group-hover:opacity-20" : ""}`}><div className="flex items-start gap-2.5"><span className={`grid size-8 shrink-0 place-items-center rounded-lg ${colors.soft} ${colors.icon}`}><NodeIcon kind={node.kind} /></span><span className="min-w-0 flex-1"><span className="block truncate text-[11px] font-black text-slate-950">{node.title}</span><span className="mt-0.5 block truncate text-[9px] font-semibold text-slate-400">{node.subtitle}</span></span><GripVertical className="size-3.5 text-slate-300" /></div><div className="mt-auto flex items-center justify-between border-t border-slate-100 pt-2 text-[8px] font-black uppercase tracking-[0.1em]"><span className={executionStatus === "running" ? "text-indigo-700" : executionStatus === "success" ? "text-emerald-700" : executionStatus === "failed" ? "text-rose-700" : "text-slate-400"}><span className={`mr-1.5 inline-block size-1.5 rounded-full ${executionStatus === "running" ? "animate-pulse bg-indigo-500" : executionStatus === "success" ? "bg-emerald-500" : executionStatus === "failed" ? "bg-rose-500" : colors.dot}`} />{executionStatus === "idle" ? "Ready" : executionStatus}</span><span className="text-slate-400">{node.kind === "trigger" ? "Trigger" : node.kind === "condition" ? "Rule" : "Action"}</span></div></div>{isConnectionTarget ? <span className="pointer-events-none absolute inset-0 grid place-items-center"><span className="inline-flex items-center gap-1.5 rounded-lg bg-indigo-600 px-3 py-2 text-[10px] font-black text-white shadow-lg"><ArrowRight className="size-3.5" />Connect here</span></span> : null}{node.kind !== "trigger" ? <span className={`absolute -left-2 top-1/2 size-4 -translate-y-1/2 rounded-full border-[3px] border-white shadow-sm ${isConnectionTarget ? "animate-pulse bg-indigo-600" : "bg-slate-400"}`} /> : null}<button aria-label={`Connect from ${node.title}`} className={`absolute -right-4 top-1/2 grid h-9 min-w-9 -translate-y-1/2 place-items-center rounded-full border-4 border-white shadow-md transition hover:scale-110 focus:outline-none focus:ring-4 focus:ring-indigo-200 ${isConnectionSource ? "animate-pulse bg-indigo-600 text-white" : "bg-slate-700 text-white hover:bg-indigo-600"}`} onClick={(event) => { event.stopPropagation(); onConnect(); }} onPointerDown={(event) => event.stopPropagation()} title={`Connect from ${node.title}`} type="button"><ArrowRight className="size-4" /></button><button aria-label={`Add next node after ${node.title}`} className="absolute -bottom-3 left-1/2 hidden h-6 -translate-x-1/2 items-center gap-1 rounded-full border border-indigo-200 bg-white px-2 text-[8px] font-black text-indigo-700 shadow-sm group-hover:flex" onClick={(event) => { event.stopPropagation(); onOpenPicker(); }} onPointerDown={(event) => event.stopPropagation()} type="button"><Plus className="size-3" />Next</button></div>;
}

function Inspector({ node, onChange, onClose, onDelete, onDuplicate, onRename, onTab, onTest, salesPeople, attendanceSessions, workshops, status, tab }: { node: WorkflowNode; onChange: (key: string, value: ConfigValue) => void; onClose: () => void; onDelete: () => void; onDuplicate: () => void; onRename: (title: string) => void; onTab: (tab: InspectorTab) => void; onTest: () => void; salesPeople: WorkflowSalesPerson[]; attendanceSessions: WorkflowAttendanceSession[]; workshops: WorkflowWorkshop[]; status: ExecutionNodeStatus; tab: InspectorTab }) {
  return <aside className="flex h-full w-full flex-col overflow-hidden bg-white"><div className="shrink-0"><div className="flex items-center gap-2 border-b border-slate-200 px-4 py-3"><span className={`grid size-9 place-items-center rounded-xl ${colorByKind[node.kind].soft} ${colorByKind[node.kind].icon}`}><NodeIcon kind={node.kind} /></span><div className="min-w-0 flex-1"><p className="truncate text-xs font-black text-slate-950">{node.title}</p><p className="mt-0.5 flex items-center gap-1 text-[9px] font-black uppercase tracking-[0.12em] text-emerald-700"><span className="size-1.5 rounded-full bg-emerald-500" />Node ready</p></div><button aria-label="Close inspector" className="rounded-lg p-1.5 text-slate-400 hover:bg-slate-100 hover:text-slate-700" onClick={onClose} type="button"><PanelRightClose className="size-4" /></button></div><div className="flex border-b border-slate-200 px-2">{(["parameters", "settings", "output"] as InspectorTab[]).map((item) => <button aria-selected={tab === item} className={`flex-1 border-b-2 px-1 py-3 text-[10px] font-black capitalize ${tab === item ? "border-indigo-500 text-indigo-700" : "border-transparent text-slate-400 hover:text-slate-700"}`} key={item} onClick={() => onTab(item)} role="tab" type="button">{item === "parameters" ? "Parameters" : item === "settings" ? "Settings" : "Test & output"}</button>)}</div></div><div className="min-h-0 flex-1 overflow-y-auto p-4">{tab === "parameters" ? <ParametersPanel attendanceSessions={attendanceSessions} workshops={workshops} node={node} onChange={onChange} onRename={onRename} onTest={onTest} salesPeople={salesPeople} /> : null}{tab === "settings" ? <SettingsPanel node={node} onChange={onChange} /> : null}{tab === "output" ? <OutputPanel node={node} onTest={onTest} status={status} /> : null}<div className="mt-4 grid grid-cols-2 gap-2"><button className="workflow-button-secondary justify-center" onClick={onDuplicate} type="button"><Copy className="size-4" />Duplicate</button><button className="inline-flex h-10 items-center justify-center gap-2 rounded-xl border border-rose-200 bg-white px-3 text-xs font-black text-rose-700 hover:bg-rose-50" onClick={onDelete} type="button"><Trash2 className="size-4" />Delete</button></div></div></aside>;
}
