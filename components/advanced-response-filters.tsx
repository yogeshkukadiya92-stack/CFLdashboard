      ...
      {open ? (
        <div aria-modal="true" className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/45 p-4 pt-[8vh] backdrop-blur-sm" role="dialog">
          ...
          <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
            ...
            <FilterField label="Confirmation"><select className={fieldClass} onChange={(event) => patch({ confirmationStatus: event.target.value })} value={filters.confirmationStatus || "all"}><option value="all">All confirmation statuses</option><option value="confirmed">Confirmed</option><option value="pending">Pending</option><option value="not_confirmed">Not confirmed</option><option value="callback">Call back</option><option value="no_answer">No answer</option><option value="cancelled">Cancelled</option><option value="carried_forward">Carried forward</option><option value="repeater">Repeater</option></select></FilterField>
            <FilterField label="MFW enrollment"><select className={fieldClass} onChange={(event) => patch({ mfwSyncStatus: event.target.value })} value={filters.mfwSyncStatus || "all"}><option value="all">All MFW statuses</option><option value="synced">Enrolled</option><option value="pending">Pending enrollment</option><option value="failed">Enrollment failed</option><option value="not_required">Not enabled / required</option></select></FilterField>
            <FilterField label="WhatsApp message"><select className={fieldClass} onChange={(event) => patch({ whatsappStatus: event.target.value })} value={filters.whatsappStatus || "all"}><option value="all">All message statuses</option><option value="sent">Sent</option><option value="failed">Failed</option><option value="not_configured">Not configured</option><option value="not_sent">Not sent</option></select></FilterField>
